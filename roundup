...
export formatter=documentation
...
roundup_summarize() {
    ...
    ntests=0 pending=0 passed=0 failed=0
    stack_trace="" pendings_name=""

    while read state name; do
        # human name
        _name=$(echo $name | sed -e 's/_/ /g')
            ...
            tap)
                case $state in
                    ...
                    p)
                        passed=$((passed + 1))
                        ntests=$((ntests + 1))
                        echo "ok $ntests - $_name"
                        ;;
                    f)
                        failed=$((failed + 1))
                        ntests=$((ntests + 1))
                        echo "not ok $ntests - $_name"
                        ;;
                esac
        esac
    done
